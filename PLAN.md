...
- **K 线走势**（左侧，lg:col-span-2）：
  ...
  - 4 个周期：1月/3月/6月/1年
  - IndexedDB 缓存 + 手动刷新 + 15s 安全超时 + 更新时间显示
- **重仓股穿透**（左侧，lg:col-span-2）：
  - AKShare fund_portfolio_hold_em，缓存 2h，手动刷新
- **Prompt 模板**（右侧，lg:col-span-1）：
  - 诊断/调仓/K 线增强三种模板
  - 传入行情 + ETF 映射 + 重仓股 + 告警数据
  - 一键复制到剪贴板
...
| Phase 13 | 🔜 待开始 | 通知系统增强：Web Push 定时扫描 + 飞书通知 |
| Phase 14 | 🔜 待开始 | 存储扩展：Notion 适配器实现 |
| Phase 15 | 🔜 待开始 | 数据同步：多设备数据同步方案 |
| Phase 16 | 🔜 待开始 | 组合风险分析：相关性矩阵、行业集中度、最大回撤 |
