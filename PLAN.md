...
### Phase 3：数据看板 + 数据源接入 ✅
- 看板概览卡片（总市值、盈亏、收益率、今日涨跌、基金数）
- 持仓分布图表（饼图-类型分布、柱状图-领域分布）
- 净值走势图（Recharts，支持基金选择和时间范围切换）
- Tushare 数据源适配器（fund_nav / fund_daily / fund_basic）
- 数据源服务层（Tushare → 模拟数据自动降级）
- 场外↔场内 ETF 映射设置（设置页新增 Tab）
...
| 决策项 | 结论 |
|--------|------|
| 规则范围 | **全局规则**，所有基金共用一套规则条件，不需要每只基金单独设置 |
| 份数系统 | **无限份数**，不设资金池，份数仅作为相对操作单位 |
| 触发条件 | ① 收益率/价差触发（百分比或绝对值）② 单日涨跌幅触发 ③ 定期定投 ④ 手动检查 |
| K 线形态提醒 | 结合 ETF 映射 + K 线数据，通过 AI 分析给出形态识别建议（如金叉/死叉/突破） |
| 执行方式 | **A 阶段（当前）**：手动点击检查扫描 → 面板显示提醒 → 用户决策执行 |
|         | **B 阶段（后续演进）**：Service Worker 定时后台扫描 + 推送通知 |

#### 触发条件详情

- **收益率触发**：持仓收益率（实时净值 vs 持仓成本）达到阈值 → 买入/卖出提醒
- **价差触发**：净值价差绝对值达到阈值 → 买入/卖出提醒
- **单日涨跌幅**：当日净值变动幅度达到阈值 → 提醒关注
- **定期定投**：按固定时间间隔（N 天/周/月）→ 买入提醒
- **K 线形态 AI 诊断**：手动触发，分析当前持仓的场内 ETF K 线形态，给出建议

#### 功能模块
- 规则配置面板（全局规则 CRUD）
- 规则引擎（扫描持仓 → 匹配规则 → 生成建议）
- 提醒面板（Dashboard 集成，展示当前触发的规则/建议）
- 操作日志（记录每次提醒和建议，标记已执行/忽略）
- AI K 线诊断（调用已配置的 AI API）
...
- Dashboard K 线图由折线替换为标准 SVG 蜡烛图
- 自动回退：无 AKTools 时仍显示净值折线图
- K 线数据本地 IndexedDB 缓存 + TTL 过期策略
