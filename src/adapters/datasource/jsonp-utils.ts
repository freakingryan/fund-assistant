...
const EM_BASE = FUND_WORKER_URL || API_BASES.eastmoneyFund;
const FUNDGZ_BASE = FUND_WORKER_URL || API_BASES.eastmoneyFundgz;
const FUNDSUGGEST_BASE = FUND_WORKER_URL || API_BASES.eastmoneyFundsuggest;
...
export async function fetchFundPingZhongData(
  code: string,
  timeout = 15000,
...
function parseFundHoldingsF10(text: string): FundHoldingsF10 | null {
  ...
  const regex =
    /<tr>\s*<td>(\d+)<\/td><td><a[^>]*>(\d{5,6})<\/a><\/td><td class='tol'><a[^>]*>([^<]+)<\/a><\/td>[\s\S]*?<td class='tor'>(\d+\.\d+)%/g;
 ...
 */
export async function fetchFundHoldingsF10(
  code: string,
  _timeout = 15000,
): Promise<FundHoldingsF10 | null> {
  try {
    if (import.meta.env.DEV) {
      ...
      const res = await fetch(
        `${FUND_WORKER_URL}/fundf10/FundArchivesDatas.aspx?type=jjcc&code=${code}&topline=10`,
      );
      if (res.ok) return parseFundHoldingsF10(await res.text());
    }
  } catch {
  ...
  return new Promise((resolve) => {
    ...
    (window as any)[cbName] = (data: any) => {
      ...
      try {
        ...
        const funds = datas.filter(
          (d) => d?.CATEGORY === 700 && d?.CODE && /^\d{6}$/.test(String(d.CODE)),
        );
        if (funds.length > 0) {
          const scored = funds
            .map((d) => ({
...
function extractShareClass(name: string): string {
  const n = name.replace(/[（(].*?[）)]/g, "").replace(/\s+/g, "");
  const m = n.match(/([ACE])(?:类|份额)?$/);
  return m ? m[1] : "";
}

/** 按份额后缀重新排序候选：同份额优先，无份额其次，不同份额最后 */
function sortByShareClass(
  candidates: FundCodeCandidate[],
  shareClass: string,
): FundCodeCandidate[] {
 ...
 */
export async function fetchFundCodeByName(
  name: string,
  timeout = 10000,
