...
async function searchStocksEastmoney(key: string): Promise<{ code: string; name: string }[]> {
  ...
  const url = `${API_BASES.eastmoneyFundsuggest}/FundSearch/api/FundSearchAPI.ashx?m=1&key=${encodeURIComponent(keyword)}&callback=${cbName}&_=${Date.now()}`;
...
export class StockApiAdapter implements FundDataSource {
  ...
  async fetchFundPortfolio(fundCode: string): Promise<{
    ...
    try {
      ...
      const rawCodes: { market: string; code: string }[] = (rawStockCodesNew || [])
        .map((c: string) => {
      ...
      const s = await ensureStocks();
      const results = await Promise.allSettled(codes.map((code: string) => s.auto.getStock(code)));
      ...
      const holdings = results
        ...
        .filter(
          (h): h is { code: string; name: string; ratio: number; value: number } => h !== null,
        )
        .slice(0, 10);
      ...
      console.error("[fetchFundPortfolio] error:", e);
      return null;
    }
  }
    ...
    try {
      ...
      try {
        ...
        const scored = stocksList.map((stock, i: number) => ({
          ...
          nameMatch: uniqueThematicKeywords.some((kw) =>
            (allEtfs.get(etfCodes[i])?.name || "").includes(kw),
          )
            ? 50
            : 0,
          // 同一基金公司加分（如华宝场外 → 华宝场内 ETF）
          companyMatch:
            otcCompany && (allEtfs.get(etfCodes[i])?.name || "").includes(otcCompany) ? 80 : 0,
        }));
        // 按(成交额 + 名称匹配 + 公司匹配)降序排列
        // 成交额相等时，公司匹配优先；都相等时，名称匹配优先
        scored.sort(
          (a, b) =>
            b.amount + b.nameMatch + b.companyMatch - (a.amount + a.nameMatch + a.companyMatch),
        );
        const best = scored[0];
