 ...
 * 4. 解析 JSON 结果返回
 */
import { getDefaultAI, callAI } from "./ai";
...
function buildAnalysisPrompt(req: KlineAnalysisRequest): string {
  ...
  const rows = recentData.map(
    (d) =>
      `${d.date} | ${d.open.toFixed(4)} | ${d.close.toFixed(4)} | ${d.high.toFixed(4)} | ${d.low.toFixed(4)} | ${d.volume || 0}`,
  );
...
export async function analyzeKline(
  ...
  try {
    ...
    const json = extractJsonFromLLM(response);
...
// ─── AI API 调用（复用 services/ai 的 callAI） ────────
