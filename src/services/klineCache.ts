...
export async function getKlineCache(code: string, period: string): Promise<KLineData[] | null> {
  return getCache<KLineData[]>(
    `k_${KLINE_CACHE_VERSION}_${code}__${period}`,
    getKlineCacheTTL(period),
  );
}

export async function setKlineCache(
  code: string,
  period: string,
  data: KLineData[],
): Promise<void> {
...
export async function setQuotesCache(
  codes: string[],
