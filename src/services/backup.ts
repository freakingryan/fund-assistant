...
export async function exportAllData(): Promise<BackupData> {
  ...
  const sanitizedSettings = settings.map((s) => ({
    ...
    storage:
      s.storage?.type === "notion" && s.storage.notion
...
const GIST_API = API_BASES.githubGists;
...
export async function verifyGistToken(token: string): Promise<{ ok: boolean; msg: string }> {
  try {
    const res = await fetch(API_BASES.githubUser, {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: "application/vnd.github+json",
      },
    ...
    if (!res.ok) {
      ...
      if (res.status === 401)
        return {
          ok: false,
          msg: "Token 无效或已过期。注意：Token 需在 Scopes 中勾选 gist（支持 classic 和 fine-grained 两种类型，生成后立即复制，页面关闭后不再显示）",
        };
...
export async function findFundGist(token: string): Promise<string | null> {
  ...
  const found = gists.find(
    (g: any) => g.files?.[GIST_FILENAME] || g.description === GIST_DESCRIPTION,
  );
