...
6. 数据质量校验：若发现单日 nextPct 超过 ±50%（如 -67% 这类单位换算异常），或 buckets 分桶与"最近样本"中的评分分布明显矛盾（如样本大量出现 60-74 分但 60-70 区间 count 为 0），必须首先明确指出数据质量问题，而非直接采信异常值继续分析；并建议先修数据再重算准确率。
