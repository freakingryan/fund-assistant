...
import type { BucketStat, SourceAccuracy } from "./stats";
...
export interface ScoreSnapshot {
  ...
  /** 快照时刻沪深300(510300) 近 60 日收益(%)，用于事后剥离市场 beta、算 alpha */
  regimeMomentum60?: number | null;
...
export interface AiBacktestAnalysis {
  ...
  context: {
    ...
    /** 方向性覆盖率：有买/卖押注的样本占已结算比例；偏低说明大量为持有建议 */
    ...
    /** 按数据源(etf/nav/unknown)拆分的方向性统计，用于对比场内ETF与净值基金表现差异 */
    bySource: Record<"etf" | "nav" | "unknown", SourceAccuracy>;
  };
  /** AI 诊断出的算法薄弱环节（中文短句） */
  weaknesses: string[];
  /** AI 给出的调参/策略建议（中文短句） */
