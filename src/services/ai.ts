...
async function callAI(
  ...
  const endpoints: Record<string, string> = {
    deepseek: baseURL || API_BASES.deepseek,
    openai: baseURL || API_BASES.openai,
    groq: baseURL || API_BASES.groq,
    openrouter: baseURL || API_BASES.openrouter,
    agnes: baseURL || API_BASES.agnes,
    google: baseURL || `${API_BASES.google}/${model || "gemini-2.0-flash"}:generateContent`,
    custom: baseURL || API_BASES.custom,
  };
...
function classifySector(name: string, type: string): string {
  ...
  if (
    ...
    return "consumer";
  if (n.includes("医") || n.includes("药") || n.includes("健康") || n.includes("生物"))
    return "healthcare";
  if (
    n.includes("新能源") ||
    n.includes("光伏") ||
  ...
  if (n.includes("债券") || n.includes("债") || n.includes("纯债") || n.includes("中短债"))
    return "bond_market";
  if (
    n.includes("全球") ||
    n.includes("海外") ||
 ...
 */
export async function recommendEtfMappingFix(mapping: {
  otcCode: string;
  otcName: string;
  exchangeCode: string;
  exchangeName: string;
  ...
  try {
    ...
    const filtered = (raw || [])
      ...
      .filter((r) => SIX_DIGIT.test(r.code) && /ETF|基金|LOF|指数|联接|篮|份额|主题/i.test(r.name));
    const withLiquidity = await Promise.all(
      filtered.slice(0, 10).map(async (r) => {
        try {
          ...
          const liquidity =
            k && k.length > 0
              ? k.reduce((sum: number, b: { volume?: number }) => sum + (b.volume || 0), 0) /
                k.length
              : 0;
