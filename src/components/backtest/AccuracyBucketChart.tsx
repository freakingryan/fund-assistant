...
import { CURSOR_FILL, UP_COLOR, DOWN_COLOR } from "@/lib/chart-colors";
    ...
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={data} margin={{ top: 16, right: 12, bottom: 20, left: 0 }}>
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="rounded-md border bg-popover text-popover-foreground px-2 py-1.5 text-[10px] shadow-md">
                ...
                <p>
                  命中率 <span className="font-mono">{p.hitRate}%</span>
                </p>
                <p className="text-muted-foreground">
                  方向性样本 {p.count} · 平均次日 {p.avgNext >= 0 ? "+" : ""}
                  {p.avgNext.toFixed(2)}%
                </p>
              </div>
            );
        ...
        <Bar dataKey="hitRate" radius={[4, 4, 0, 0]}>
          {data.map((d, i) => (
            <Cell key={i} fill={d.hitRate != null && d.hitRate >= 50 ? UP_COLOR : DOWN_COLOR} />
          ))}
          <LabelList
            dataKey="hitRate"
            position="top"
