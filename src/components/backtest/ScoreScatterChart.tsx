...
import { RESULT_COLORS, NEUTRAL_GRAY } from "@/lib/chart-colors";

const COLOR = RESULT_COLORS;

export default function ScoreScatterChart({ snapshots }: { snapshots: ScoreSnapshot[] }) {
  const data = snapshots
    .filter(
      (s) =>
        s.nextChangePct != null &&
        (s.outcome === "correct" || s.outcome === "wrong" || s.outcome === "neutral"),
    )
    .map((s) => ({
      score: s.score,
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="rounded-md border bg-popover text-popover-foreground px-2 py-1.5 text-[10px] shadow-md">
                ...
                <p>
                  评分 <span className="font-mono">{p.score}</span> · 次日{" "}
                  <span className={`font-mono ${p.next >= 0 ? "text-up" : "text-down"}`}>
                    {p.next >= 0 ? "+" : ""}
                    {p.next.toFixed(2)}%
                  </span>
                </p>
                <p className="text-muted-foreground">
                  结果：
                  {p.outcome === "correct" ? "命中" : p.outcome === "wrong" ? "未命中" : "中性"}
                </p>
              </div>
            );
          }}
