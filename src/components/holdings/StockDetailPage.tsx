...
import { asOfFromKlines, asOfFromQuotes } from "@/lib/dataTime";
...
export default function StockDetailPage() {
  ...
  const quoteAsOf = useMemo(() => asOfFromQuotes(quote ? [quote] : []), [quote]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!cancelled && cached?.length) {
        ...
        getKlineCacheTime(cacheKey, period).then((ts) => {
          if (ts) {
      ...
      if (!cancelled) {
        if (data.length > 0) {
          setKlineCache(cacheKey, period, data);
          setKlineAsOf(asOfFromKlines(data));
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="text-center py-16">
            <p className="text-muted-foreground">股票代码缺失</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const quoteColor = quote?.dailyChange != null ? pnlColor(quote.dailyChange) : "text-foreground";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="py-4">
          ...
          {quote && <DataAsOf asOf={quoteAsOf} fetchedAt={quoteFetchedAt} />}
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <KlineChartCard
            ...
            klineDetectedPatterns={klineDetectedPatterns}
            onHover={setHoveredKlineIndex}
          ...
          <KlinePatternCard
            klineData={klineData}
            klineDetectedPatterns={klineDetectedPatterns}
            klinePatterns={klinePatterns}
            klineAnalysis={klineAnalysis}
            klineAnalyzing={klineAnalyzing}
            klineAnalysisError={klineAnalysisError}
            ...
            onAnalyzeKline={handleAnalyzeKline}
            onGenerateKlinePrompt={handleGenerateKlinePrompt}
          />
          <DecisionAdvisorCard
            klines={klineData}
          ...
          <details className="group rounded-lg border border-border/50 bg-muted/20 px-3 py-2">
            ...
            <div className="mt-3 space-y-4">
              <SignalScoreCard
                ...
                setShowSignalDetail={setShowSignalDetail}
              />
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">数据说明</CardTitle>
            </CardHeader>
