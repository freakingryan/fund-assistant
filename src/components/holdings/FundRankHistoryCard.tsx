...
import { AMBER, UP_COLOR, DOWN_COLOR, CURSOR_STROKE } from "@/lib/chart-colors";
...
function lineColor(pct: number | null | undefined): string {
  ...
  if (pct <= 25) return UP_COLOR; // 红（靠前）
  if (pct > 50) return DOWN_COLOR; // 绿（靠后）
  return AMBER;
}
      ...
      <CardContent>
        {!config.enabled ? (
          <div className="flex items-start gap-2 text-[11px] text-muted-foreground py-3">
            ...
            <span>
              同类排名走势需开启东财增强。到「设置 →
              数据源」打开「东财资金面增强」后，即可展示该基金在同类基金中的近三月排名走势。
            </span>
          </div>
        ) : loading ? (
          ...
          <div className="space-y-3">
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded-md border bg-popover text-popover-foreground px-2 py-1.5 text-[10px] shadow-md">
                        ...
                        <p>
                          同类百分位{" "}
                          <span className="font-mono">前 {p.percentile.toFixed(1)}%</span>
                        </p>
            ...
            <p className="text-[10px] text-muted-foreground">
              纵轴为同类近三月排名百分位（越小越靠前，已反向显示：越靠上越好）；虚线为同类中位（前
              50%）。
            </p>
          </div>
        )}
