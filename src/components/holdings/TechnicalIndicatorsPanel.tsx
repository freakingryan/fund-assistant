...
): { period: string; value: number | null } | null {
  if (!rec) return null;
...
function toneClass(tone: SignalDirection): string {
      ...
      <CardContent className="space-y-3">
        {!ohlcAvailable && (
          <div className="flex items-start gap-1.5 rounded-md bg-muted/40 px-2.5 py-1.5 text-[11px] text-muted-foreground">
            ...
            <span>
              当前为基金净值走势（无盘中区间），仅显示 BIAS/ROC；切换「场内 ETF 真实 K 线」可查看
              KDJ/WR/CCI/ATR/DMI/SAR/KC 等完整指标。
            </span>
          </div>
        )}
          ...
          <div className="space-y-1.5">
            ...
            <ul className="space-y-1">
              {signals.map((s, i) => (
                <li
                  key={`${s.type}-${s.date}-${i}`}
                  className="flex items-center gap-2 text-xs rounded-md px-1.5 py-1 transition-colors hover:bg-muted/50"
                >
                  <span
                    className={`h-1.5 w-1.5 rounded-full shrink-0 ${s.direction === "up" ? "bg-up" : "bg-down"}`}
                  />
                  <span className={`font-medium ${s.direction === "up" ? "text-up" : "text-down"}`}>
                    {s.label}
                  </span>
...
function KdjStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  const tone: SignalDirection = j == null ? "neutral" : j > 80 ? "down" : j < 20 ? "up" : "neutral";
  return (
    <Stat
      label="KDJ (K/D/J)"
      value={`${fmt(k)} / ${fmt(d)} / ${fmt(j)}`}
      sub="J>80 超买 · J<20 超卖"
      tone={tone}
    />
  );
...
function WrStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  if (!f) return null;
  const tone: SignalDirection =
    f.value == null ? "neutral" : f.value < 20 ? "down" : f.value > 80 ? "up" : "neutral";
  return (
    <Stat label={`WR(${f.period})`} value={fmt(f.value)} sub="<20 超买 · >80 超卖" tone={tone} />
  );
}

function CciStat({ snap }: { snap: IndicatorSnapshot }) {
  if (snap.cci == null) return null;
  const tone: SignalDirection = snap.cci > 100 ? "down" : snap.cci < -100 ? "up" : "neutral";
...
function BiasStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  if (!f) return null;
  const tone: SignalDirection =
    f.value == null ? "neutral" : f.value > 5 ? "down" : f.value < -5 ? "up" : "neutral";
...
function DmiStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  const tone: SignalDirection = pdi == null || mdi == null ? "neutral" : pdi > mdi ? "up" : "down";
  return (
    <Stat
      label="DMI (+DI/-DI/ADX)"
      value={`${fmt(pdi)} / ${fmt(mdi)} / ${fmt(adx)}`}
      sub="ADX>25 趋势强"
      tone={tone}
    />
  );
...
function SarStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  const tone: SignalDirection =
    snap.sar.trend === 1 ? "up" : snap.sar.trend === -1 ? "down" : "neutral";
...
function RocStat({ snap }: { snap: IndicatorSnapshot }) {
  ...
  const tone: SignalDirection = snap.roc.roc == null ? "neutral" : snap.roc.roc > 0 ? "up" : "down";
