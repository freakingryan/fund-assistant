...
export default function KlinePatternCard({
  ...
  /** 算法检测列表折叠状态：默认仅展示前 10 条，点击「展开剩余 N 条」展开全部 */
  const [patternsExpanded, setPatternsExpanded] = useState(false);
  /** 算法检测按 K 线索引倒序（最新在前） */
  const sortedPatterns = [...klineDetectedPatterns].sort((a, b) => b.index - a.index);
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-6 text-center">
              ...
              {etfKlineError ? (
                <p className="text-[10px] text-orange-500 max-w-xs leading-relaxed">
                  {etfKlineError}
                </p>
              ) : (
                <p className="text-[10px] text-muted-foreground/70 max-w-xs leading-relaxed">
                  K 线形态分析需基于真实 K
                  线的开/收/高/低（OHLC）数据；净值序列无盘中区间，无法准确识别形态（易误判为十字星）。
    ...
    <Card className="card-hover">
      ...
      <CardContent className="space-y-3">
        ...
        <div>
          ...
          {klineDetectedPatterns.length > 0 ? (
            <div className="space-y-0.5">
              {(patternsExpanded ? sortedPatterns : sortedPatterns.slice(0, 10)).map((p, i) => {
                const isHighlighted =
                  hoveredKlineIndex === p.index || selectedKlineIndex === p.index;
                const tipKline = klineData[p.index];
                const tipStart =
                  p.isMultiCandle && p.candleCount > 1
                    ? klineData[p.index - p.candleCount + 1]
                    : null;
                const tipChange =
                  tipKline && tipKline.open
                    ? ((tipKline.close - tipKline.open) / tipKline.open) * 100
                    : 0;
                return (
                  <div
                    ...
                    onMouseEnter={() => {
                      onPatternHover?.(p.index);
                      if (!isTouch) setOpenTip(p.index);
                    }}
                    onMouseLeave={() => {
                      onPatternHover?.(null);
                      if (!isTouch) setOpenTip(null);
                    }}
                    onClick={(e) => {
                      e.stopPropagation();
                      onPatternSelect?.(p.index);
                      if (isTouch) setOpenTip((prev) => (prev === p.index ? null : p.index));
                    }}
                  >
                    <span
                    ...
                    <span className="text-[10px] text-muted-foreground shrink-0">
                      {p.isMultiCandle ? `${p.candleCount}K` : "单K"} ·{" "}
                    ...
                    {openTip === p.index && tipKline && (
                      <div className="absolute z-50 left-2 right-2 bottom-full mb-1 rounded-md border border-border bg-popover text-popover-foreground shadow-lg p-2 text-[10px] leading-relaxed">
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
                          ...
                          <span className={pnlColor(tipChange)}>
                            涨跌：{tipChange >= 0 ? "+" : ""}
                            {tipChange.toFixed(2)}%
                          </span>
              ...
              {sortedPatterns.length > 10 && (
                ...
                >
                  {patternsExpanded ? "收起" : `展开剩余 ${sortedPatterns.length - 10} 条`}
                </button>
              )}
            </div>
          ) : (
          ...
          <div className="space-y-2 border-t pt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-muted-foreground">
                置信度:{" "}
                {klineAnalysis.confidence === "high"
                  ? "高"
                  : klineAnalysis.confidence === "medium"
                    ? "中"
                    : "低"}
              </span>
            </div>
            ...
            {klineAnalysisError && (
              <p className="text-[10px] text-orange-500">
                注: {klineAnalysisError}，仅显示算法检测结果
              </p>
            )}
          </div>
        )}
        ...
        <div className="border-t pt-2">
          ...
          >
            <span
              className={`inline-block transition-transform ${glossaryOpen ? "rotate-90" : ""}`}
            >
              ▶
            </span>
            K 线形态术语说明
          ...
          {glossaryOpen && (
            <div className="mt-2 text-[10px] text-muted-foreground leading-relaxed space-y-2">
              <div className="p-2 rounded bg-muted/20">
                ...
                <p>
                  基于单根 K
                  线的形状判断价格行为。常见形态包括十字星（多空平衡）、锤子线（下方支撑）、光头光脚（单边行情）等。
                </p>
                <div className="mt-1 flex flex-wrap gap-1">
                  {[
                    ...
                    <span key={t} className="px-1 py-0.5 rounded bg-muted/40 text-[9px]">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-2 rounded bg-muted/20">
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  <span className="px-1 py-0.5 rounded bg-up/10 text-up text-[9px] font-medium">
                    看涨吞没
                  </span>
                ...
                <div className="mt-0.5 flex flex-wrap gap-1">
                  <span className="px-1 py-0.5 rounded bg-down/10 text-down text-[9px] font-medium">
                    看跌吞没
                  </span>
                  <span className="text-[9px] text-muted-foreground self-center">
                    后阴包前阳，强势反转看跌
              ...
              <div className="p-2 rounded bg-muted/20">
                ...
                <div className="mt-1 space-y-0.5">
                  <div className="flex flex-wrap gap-1 items-start">
                    <span className="px-1 py-0.5 rounded bg-up/10 text-up text-[9px] font-medium shrink-0">
                      晨星
                    </span>
                  ...
                  <div className="flex flex-wrap gap-1 items-start">
                    <span className="px-1 py-0.5 rounded bg-down/10 text-down text-[9px] font-medium shrink-0">
                      暮星
                    </span>
                    <span className="text-[9px] text-muted-foreground">
                      大阳→小实体(星)→大阴，顶部反转
              ...
              <div className="p-2 rounded bg-muted/20">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 text-[9px]">
                  <span>
                    <span className="text-up font-medium">红色</span>=涨 (阳线)
                  </span>
                  <span>
                    <span className="text-down font-medium">绿色</span>=跌 (阴线)
                  </span>
                  <span>实体 = 开收盘价差</span>
