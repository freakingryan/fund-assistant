...
export default function KlineChartCard({
  ...
  const navLinePoints = useMemo(() => {
    ...
    return klineData
      .map(
        (d, i) =>
          `${i * xStep},${NAV_CHART_BASELINE - ((d.close - min) / range) * NAV_CHART_SCALE}`,
      )
      .join(" ");
  }, [klineData]);

  const hasValidEtfQuote = etfQuote && etfQuote.nav && etfQuote.nav > 0.001;
  const etfQuoteChangeColor = hasValidEtfQuote
    ? pnlColor(etfQuote.dailyChange)
  ...
  const chartTitle = isStock ? "K 线走势" : showCandlestick ? "场内 ETF 真实 K 线" : "基金净值走势";
      ...
      <CardHeader className="pb-2">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
          <div className="flex flex-col gap-1">
                ...
                {hasValidEtfQuote ? (
                  <>
                    ...
                    <span className={`font-mono font-medium ${etfQuoteChangeColor}`}>
                      {etfQuote.dailyChange >= 0 ? "+" : ""}
                      {etfQuote.dailyChange.toFixed(2)}%
                    </span>
                  </>
                ) : (
            ...
            {klineData.length > 0 && <DataAsOf asOf={asOf} fetchedAt={fetchedAt} inline />}
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <>
                <button
                  ...
                  className={`text-[10px] px-1.5 py-0.5 rounded border transition-colors cursor-pointer ${
                    showMA
                      ? "bg-amber-50 border-amber-300 text-amber-700 dark:bg-amber-950 dark:border-amber-700 dark:text-amber-400 font-medium"
                ...
                <button
                  ...
                  className={`text-[10px] px-1.5 py-0.5 rounded border transition-colors cursor-pointer ${
                    showBollinger
                      ? "bg-blue-50 border-blue-300 text-blue-700 dark:bg-blue-950 dark:border-blue-700 dark:text-blue-400 font-medium"
                      : "border-muted text-muted-foreground hover:bg-muted/50"
                  }`}
                >
                  BOLL
            ...
            <Select value={period} onValueChange={setPeriod}>
              <SelectTrigger className="h-7 text-xs w-[62px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        {etfCode && (
          <div className="flex items-center gap-2 mb-3">
            ...
            <Label htmlFor="etf-kline" className="text-xs cursor-pointer">
              {useEtfKline ? "场内 ETF 真实 K 线" : "基金净值走势"}{" "}
              <span className="text-[10px] text-muted-foreground">（{etfCode}）</span>
            </Label>
          </div>
        )}
        {klineLoading ? (
          ...
          <>
            ...
            <div className="w-full">
              <CandlestickChart
                ...
                externalHighlightIndex={externalHighlightIndex}
            ...
            {(showMA || showBollinger) && klineData.length > 1 && (
              <div className="mt-1">
                ...
                >
                  <span
                    className={`inline-block transition-transform ${klineIndicatorInfoOpen ? "rotate-90" : ""}`}
                  >
                    ▶
                  </span>
                  MA / BOLL 技术指标说明
                  ...
                  <div className="mt-1.5 text-[10px] text-muted-foreground leading-relaxed space-y-2">
                    ...
                    {showMA &&
                      (() => {
                        ...
                        const ma20Start =
                          klineData[Math.min(19, klineData.length - 1)]?.date || "?";
                        ...
                        return (
                          <div className="p-2 rounded bg-muted/20">
                            ...
                            <p>
                              过去 N 个交易日收盘价的算术平均值，用于识别趋势方向和支撑/阻力位。
                            </p>
                            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
                              {MA_LABELS.slice(0, 3).map((label, i) => (
                                <span key={label} className="text-2xs">
                                  <span className="font-medium" style={{ color: MA_COLORS[i] }}>
                                    {label}
                                  </span>{" "}
                            ...
                            <p className="text-2xs text-muted-foreground/70 mt-0.5">
                              用法：价格在均线上方→短期偏强，下方→短期偏弱。MA5/10
                              金叉=买入信号，死叉=卖出信号。
                            </p>
                          </div>
                        );
                        ...
                        return (
                          <div className="p-2 rounded bg-muted/20">
                            ...
                            <p className="text-[9px] text-muted-foreground/70 mt-0.5">
                              用法：带宽变宽=波动加大，变窄（收口）=即将变盘。价格触及上轨=超买，触及下轨=超卖。
                            </p>
                          </div>
                        );
                      })()}
