...
export default function AddFundDialog() {
  ...
  const [etfLookupResults, setEtfLookupResults] = useState<
    Map<string, { exchangeCode: string; exchangeName: string } | null>
  >(new Map());
  // 手动补充 ETF 映射：otcCode → 用户输入 / 搜索结果 / 加载态
  const [manualInput, setManualInput] = useState<Map<string, string>>(new Map());
  const [manualResults, setManualResults] = useState<Map<string, { code: string; name: string }[]>>(
    new Map(),
  );
  ...
  const searchCode = useMemo(
    () => (/^\d{6}$/.test(searchQuery.trim()) ? searchQuery.trim() : ""),
    [searchQuery],
  );
  ...
  // 搜索防抖：复用通用 hook，避免重复手写 setTimeout
  ...
  useEffect(() => {
    ...
    dataSourceService
      .searchFunds(debouncedSearchQuery.trim())
      .then((results) => {
  ...
  const handleEtfMappingLookup = async () => {
    ...
    const allCodes = [
      ...new Set([...codesFromRows, ...(localSearchCode ? [localSearchCode] : [])]),
    ];

    if (allCodes.length === 0) {
      setError("请先输入或搜索基金代码");
      return;
    }
    ...
    try {
      ...
      for (const [i, result] of results.entries()) {
        ...
        if (result.status === "fulfilled" && result.value) {
          succeeded++;
          newResults.set(code, {
            exchangeCode: result.value.exchangeCode,
            exchangeName: result.value.exchangeName,
          });
          addEtfMapping(
            result.value.otcCode,
            result.value.otcName,
            result.value.exchangeCode,
      ...
      toast({
        ...
        message: `ETF 映射查询完成：${succeeded} 条成功${failed > 0 ? `，${failed} 条未找到映射` : ""}`,
      });
    } catch (err) {
      setError(String(err));
    }
  ...
  const handleManualConfirm = async (otcCode: string, result: { code: string; name: string }) => {
    ...
    await setEtfMappingCache(otcCode, {
      ...
      exchangeName: result.name,
    });
    setEtfLookupResults((prev) =>
      new Map(prev).set(otcCode, { exchangeCode: result.code, exchangeName: result.name }),
    );
    setManualInput((prev) => {
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-xl max-h-[90vh] overflow-auto">
        ...
        <div className="space-y-4">
            ...
            {rows.map((row, idx) => (
              <div key={row.key} className="rounded-md border bg-muted/10 p-3 space-y-2">
                ...
                <div className="flex gap-2 items-start">
                  <span className="text-[10px] text-muted-foreground shrink-0 w-12 pt-1.5">
                    方式一
                  </span>
                  <div className="flex-1 space-y-0.5">
                    <Input
                      ...
                      className={cn(
                        "h-7 text-xs",
                        numericError(row.costNAV) &&
                          "border-destructive focus-visible:ring-destructive",
                      )}
                    />
                    {numericError(row.costNAV) && (
                      <p className="text-[9px] text-destructive leading-none">
                        {numericError(row.costNAV)}
                      </p>
                    )}
                  </div>
                ...
                <div className="flex gap-2 items-start">
                  ...
                  <div className="flex-1 space-y-0.5">
                    <Input
                      ...
                      className={cn(
                        "h-7 text-xs",
                        numericError(row.holdingAmount) &&
                          "border-destructive focus-visible:ring-destructive",
                      )}
                    />
                    {numericError(row.holdingAmount) && (
                  ...
                  <div className="flex-1 space-y-0.5">
                    <Input
                      ...
                      className={cn(
                        "h-7 text-xs",
                        numericError(row.holdingProfit, true) &&
                          "border-destructive focus-visible:ring-destructive",
                      )}
                    />
                    {numericError(row.holdingProfit, true) && (
                  ...
                  <div className="space-y-2 pt-1 border-t">
                    <div className="grid grid-cols-3 gap-2">
                      <div className="space-y-1">
                        ...
                        <Select
                          value={row.market}
                          onValueChange={(v) => updateRow(row.key, "market", v)}
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={row.sector}
                          onValueChange={(v) => updateRow(row.key, "sector", v)}
                        >
                          <SelectTrigger className="h-7 text-xs">
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20 p-2.5 space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              setManualInput((prev) =>
                                new Map(prev).set(row.code.trim(), e.target.value),
                              )
                            }
                          ...
                          <Button
                            ...
                            disabled={
                              !!manualLoading.get(row.code.trim()) ||
                              !(manualInput.get(row.code.trim()) ?? "").trim()
                            }
                          >
                            {manualLoading.get(row.code.trim()) ? (
          ...
          <Button
            ...
            disabled={totalCodes.length === 0 || submitting}
          >
